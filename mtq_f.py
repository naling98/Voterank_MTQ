...
def SIR(g, infeacted_set, infect_prob, cover_prob, max_iter):
    ...
    while len(infeacted_set) != 0 and max_iter != 0:
        ...
        for node in infeacted_set:
            ...
            nbr = np.random.choice(list(nbrs))
...
def voterank(G, topk):
    """use the voterank to get topk nodes
     ...
     """
    k_ = 1.0 / (nx.number_of_edges(G) * 2.0 / float(nx.number_of_nodes(G)))
    print k_
    result_rank = []
    voting_ability = {}
    node_scores = {}
...
time_arr=[]
func_ar=[SIR, degree, voterank, kshell]
for i in range(len(func_ar)):
    import time
    a=time.time()
    if i==0:
        SIR(G, [ch(list(G.nodes)) for _ in range(100)], 0.6, 0.6, 10)
    if i==1:
        degree(G, 10)
    if i==2:
        voterank(G, 10)
    if i==3:
        kshell(G, 10)
    b=time.time()
    time_arr.append(round(b-a, 2))

for i in range(len(func_ar)):
    print str(func_ar[i]).split()[1]+": "+str(time_arr[i])
